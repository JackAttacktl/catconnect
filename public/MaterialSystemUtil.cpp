...
{
	IMaterial *pMaterial = NSInterfaces::g_pMaterialSystemFixed->FindMaterial( pMaterialName, pTextureGroupName, bComplain);
	Assert( pMaterial );
	Init( pMaterial );
}
...
void CMaterialReference::Init( const char *pMaterialName, KeyValues *pVMTKeyValues )
{
	...
	Shutdown();
	m_pMaterial = NSInterfaces::g_pMaterialSystemFixed->CreateMaterial( pMaterialName, pVMTKeyValues );
}

void CMaterialReference::Init( const char *pMaterialName, const char *pTextureGroupName, KeyValues *pVMTKeyValues )
{
	IMaterial *pMaterial = NSInterfaces::g_pMaterialSystemFixed->FindProceduralMaterial( pMaterialName, pTextureGroupName, pVMTKeyValues );
	Assert( pMaterial );
	Init( pMaterial );
}
...
{
	if ( m_pMaterial && NSInterfaces::g_pMaterialSystemFixed )
	{
...
{
	Shutdown();
	m_pTexture = NSInterfaces::g_pMaterialSystemFixed->FindTexture( pTextureName, pTextureGroupName, bComplain );
	if ( m_pTexture )
	{
		m_pTexture->IncrementReferenceCount();
	}
...
void CTextureReference::InitProceduralTexture( const char *pTextureName, const char *pTextureGroupName, int w, int h, ImageFormat fmt, int nFlags )
{
	...
	m_pTexture = NSInterfaces::g_pMaterialSystemFixed->CreateProceduralTexture( pTextureName, pTextureGroupName, w, h, fmt, nFlags );
...
void CTextureReference::InitRenderTarget( int w, int h, RenderTargetSizeMode_t sizeMode, ImageFormat fmt, MaterialRenderTargetDepth_t depth, bool bHDR, char *pStrOptionalName /* = NULL */ )
{
	...
	m_pTexture = NSInterfaces::g_pMaterialSystemFixed->CreateNamedRenderTargetTextureEx( pStrOptionalName, w, h, sizeMode, fmt,
...
{
	if ( m_pTexture && NSInterfaces::g_pMaterialSystemFixed )
	{
