...
#ifndef ISURFACE_V30_FIXED_H
#define ISURFACE_V30_FIXED_H
...
namespace SurfaceV30
{
	...
	//-----------------------------------------------------------------------------
	class ISurfaceFixed : public IAppSystem
	{
	public:
		...
		virtual void DeleteTextureByID(int id) = 0; //missed function
		virtual int CreateNewTextureID(bool procedural = false) = 0;
#ifdef _XBOX
		virtual void DestroyTextureID(int id) = 0; //this function also exists in pc now but in different place, look below
		...
		virtual bool SetCursorAlwaysVisible(bool bVisible) = 0; //missed function, is return bool?
		...
		virtual bool SetFontGlyphSet(vgui::HFont font, const char* windowsFontName, int tall, int weight, int blur, int scanlines, int flags, int iUnk = 0, int iUnk2 = 0) = 0; //2 missed arguments
		...
		virtual int GetFontTallRequested(vgui::HFont font) = 0; //missed function
		...
		virtual void DrawTexturedPolygon(int n, Vertex_t* pVertices, bool bUnk) = 0; //missed argument
		...
		virtual void DrawFilledRectFastFade(int x0, int y0, int x1, int y1, int iUnk1, int iUnk2, unsigned int alpha0, unsigned int alpha1, bool bHorizontal) = 0; //missed function
		...
		virtual void ClearTemporaryFontCache() = 0; //missed function
		...
		/*TODO: Add these functions:
			...
			CMatSystemSurface::GetResolutionKey(void)const
			...
			CMatSystemSurface::ResetFontCaches(void)
			CMatSystemSurface::GetTextureNumFrames(int)
			CMatSystemSurface::DrawSetTextureFrame(int,int,unsigned int *)
			CMatSystemSurface::IsScreenSizeOverrideActive(void)
			CMatSystemSurface::IsScreenPosOverrideActive(void)
			...
			CMatSystemSurface::GetWebkitHTMLUserAgentString(void)
			CMatSystemSurface::Deprecated_AccessChromeHTMLController(void)
			CMatSystemSurface::SetFullscreenViewport(int,int,int,int)
			CMatSystemSurface::GetFullscreenViewport(int &,int &,int &,int &)
			CMatSystemSurface::PushFullscreenViewport(void)
			CMatSystemSurface::PopFullscreenViewport(void)
			CMatSystemSurface::SetSoftwareCursor(bool)
			CMatSystemSurface::PaintSoftwareCursor(void)
			...
			CMatSystemSurface::End3DPaint(void)
			...
			CMatSystemSurface::Reset3DPaintTempRenderTarget(void)
			...
			CMatSystemSurface::BeginSkinCompositionPainting(void)
			CMatSystemSurface::EndSkinCompositionPainting(void)
			CMatSystemSurface::~CMatSystemSurface() //here some another interface starts, we can get its name from rtti
			CMatSystemSurface::~CMatSystemSurface()
			CMatSystemSurface::InEngine(void)
			CMatSystemSurface::IsInThink(unsigned int)*/
	};

} // end namespace

#endif // ISURFACE_V30_FIXED_H
